  ...
  return (
    <section id="compare" style={{ scrollMarginTop: '24px' }}>
      <div style={{ marginBottom: '28px' }}>
        <div style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.14em', color: 'var(--toyota-red)', marginBottom: '8px' }}>SPECIFICATIONS</div>
        <h2 style={{ margin: '0 0 6px', fontSize: '32px', fontWeight: 700, letterSpacing: '-0.02em' }}>諸元比較表</h2>
            ...
            {view.vehicleRows.map((v, vi) => (
              <div key={vi} style={css(v.rowStyle)}>
                <div style={css(v.vehStyle)}>
                  ...
                  <div>
                    <div style={{ fontSize: '15px', fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--text-primary)', lineHeight: 1.2 }}>{v.name}</div>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-tertiary)', marginTop: '2px' }}>{v.type}</div>
                  </div>
                </div>
                {v.cells.map((cell, ci) => (
